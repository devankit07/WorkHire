  ...
  return (
    <main className="flex flex-col gap-10 sm:gap-20 py-10 sm:py-20">
      <section className="text-center px-4 max-w-4xl mx-auto">
        <h1 className=" text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-tight dark:text-white text-slate-400">
          Find Your Dream Job Faster
        </h1>
        <p className="mt-5 dark:text-muted-foreground text-slate-600 text-sm sm:text-lg">
          ...
          {companies.map(({ name, id, path }) => (
            <CarouselItem key={id} className="basis-1/2 sm:basis-1/4 lg:basis-1/6 flex justify-center">
              ...
              <img
                src={path}
                alt={name}
                className="h-8 sm:h-10 object-contain opacity-80 hover:opacity-100 transition dark:invert-0 invert"
              />
            </CarouselItem>
          ))}
        </CarouselContent>
